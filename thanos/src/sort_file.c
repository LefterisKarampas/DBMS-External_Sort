#include "sort_file.h"
#include "bf.h"
#include <string.h>
#include <stdio.h>

SR_ErrorCode SR_Init() {
  // Your code goes here
  BF_Init(LRU); 
  return SR_OK;
}

SR_ErrorCode SR_CreateFile(const char *fileName) {
  // Your code goes here
  SR_ErrorCode error;
  int fd;
  BF_Block *block;
  BF_Block_Init(&block);                                                          //Initialize Block
  if((error = BF_CreateFile(fileName)) != BF_OK)
  {                                 
    BF_Block_Destroy(&block);
    //BF_PrintError(error);                                                         //If creation fails
    return SR_ERROR;                                                              //Return error
  }
  if ((error = BF_OpenFile(fileName, &fd)) != BF_OK)
  {
    BF_Block_Destroy(&block);
    //BF_PrintError(error);                                                         
    return SR_ERROR;
  }
  if((error = BF_AllocateBlock(fd, block)) != BF_OK)                              //Allocate a new block
  {                             
    BF_CloseFile(fd);
    BF_Block_Destroy(&block);
    //BF_PrintError(error);                                                         //If fails 
    return SR_ERROR;                                                              //Return error
  } 
  int m=0;
  int default_block = -1;
  char *data;
  data = BF_Block_GetData(block);
  memcpy(data, "SR", strlen("SR")+1);                                             //Initialize the new file
  m += strlen("SR")+1;
  memcpy(&(data[m]), &default_block, sizeof(int));
  BF_Block_SetDirty(block);                                                       //as SR
  BF_UnpinBlock(block);                                                           
  BF_CloseFile(fd);                                                               //Close file
  BF_Block_Destroy(&block);  
  return SR_OK;
}

SR_ErrorCode SR_OpenFile(const char *fileName, int *fileDesc) {
  // Your code goes here
  SR_ErrorCode error;
  BF_Block *block;
  BF_Block_Init(&block);                                                          //Initialize Block
  if((error = BF_OpenFile(fileName, fileDesc)) != BF_OK)                          //Open File
  {                         
    BF_Block_Destroy(&block);
    //BF_PrintError(error);                                                         //If fails 
    return SR_ERROR;                                                              //Return error
  }
  int i = 0;
  if((error =BF_GetBlock(*fileDesc, i, block)) != BF_OK)                          //Get first block
  {                         
    BF_Block_Destroy(&block); 
    //BF_PrintError(error);                                                         //If fails
    return SR_ERROR;                                                              //Return error
  }
  char *data;
  data = BF_Block_GetData(block);                                                 //Get the block's data
  if(strcmp(data,"SR"))
  {                                                                               //Check if the file is SR
    BF_UnpinBlock(block);                                                         //If not SR
    BF_Block_Destroy(&block);                                                      
    return SR_ERROR;                                                              //Return error
  }
  printf("%c%c\n",data[0],data[1]);
  BF_UnpinBlock(block);                                                           //Unpin the block
  BF_Block_Destroy(&block);                                                       
  return SR_OK;
}

SR_ErrorCode SR_CloseFile(int fileDesc) {
  // Your code goes here
  SR_ErrorCode error;
  if((error = BF_CloseFile(fileDesc)) != BF_OK)                                   //Close file
  {                                    
    //BF_PrintError(error);
    return SR_ERROR;
  }
  return SR_OK;
}

SR_ErrorCode SR_InsertEntry(int fileDesc,	Record record) {
  // Your code goes here
	BF_Block *block;
	BF_Block_Init(&block);
	char *data;
	int block_num;
	int next_block = -1;
	BF_GetBlockCounter(fileDesc, &block_num);
	BF_GetBlock(fileDesc, block_num-1, block);
	data = BF_Block_GetData(block);
	if ((block_num -1) == 0)
	{
		memcpy(&(data[3]),&block_num,sizeof(int));									//number of first block , write to metadata
		BF_UnpinBlock(block);
		BF_AllocateBlock(fileDesc, block);
		data = BF_Block_GetData(block);
		int m=sizeof(int);
		int counter = 1;
		memcpy(data,&counter,sizeof(int));										//counter
		memcpy(&(data[m]), &record, sizeof(record));							//insert entry
		memcpy(&(data[BF_BLOCK_SIZE-sizeof(int)]), &next_block, sizeof(int));	//set next pointer to default
	}
	else
	{
		int counter;
		int m = sizeof(int);
		memcpy(&counter, data, sizeof(int));
		if (counter < (BF_BLOCK_SIZE - 2*sizeof(int))/ sizeof(Record))				//free space in current block
		{
			memcpy(&(data[m+counter*sizeof(record)]), &record, sizeof(Record));
			counter++;
			memcpy(data, &counter, sizeof(int));
		}
		else																		//full block, allocate new
		{
			memcpy(&(data[BF_BLOCK_SIZE-sizeof(int)]), &block_num, sizeof(int));	//update next pointer 
			BF_UnpinBlock(block);
			BF_AllocateBlock(fileDesc, block);
			data = BF_Block_GetData(block);
			counter = 1;
			memcpy(data,&counter,sizeof(int));
			memcpy(&(data[m]), &record, sizeof(Record));							//insert entry
			memcpy(&(data[BF_BLOCK_SIZE-sizeof(int)]), &next_block, sizeof(int));	//set next pointer to default
		}
	}
	BF_UnpinBlock(block);
	BF_Block_Destroy(&block);

  return SR_OK;
}

SR_ErrorCode SR_SortedFile(
  const char* input_filename,
  const char* output_filename,
  int fieldNo,
  int bufferSize
) {
  // Your code goes here

  return SR_OK;
}

SR_ErrorCode SR_PrintAllEntries(int fileDesc) {
  // Your code goes here
	BF_Block *block;
	BF_Block_Init(&block);
	Record record;
	int m = sizeof(int);
	int counter , i , y, block_num; 
	BF_GetBlockCounter(fileDesc, &block_num);
	printf("block_num %d\n",block_num);
	BF_GetBlock(fileDesc, 1, block);
	char *data;
	data = BF_Block_GetData(block);
	memcpy(&(record.name), &(data[m+sizeof(int)]), sizeof(record.name));
	printf("%s name\n",record.name);
	memcpy(&counter, data, sizeof(int));
	//for (y=1;y<block_num-1;y++)
	//{
		//BF_GetBlock(fileDesc, 1, block);
		//data = BF_Block_GetData(block);
		for (i=0;i<counter;i++)
		{
			printf("here\n");
			memcpy(&record, &(data[m+i*sizeof(Record)]), sizeof(int));
			printf("%d , %s, %s, %s\n",record.id,record.name,record.surname,record.city);
		}
		BF_UnpinBlock(block);
	//}
	BF_Block_Destroy(&block);
  return SR_OK;
}
